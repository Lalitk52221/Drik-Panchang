  ...
  return (
    <div className="w-full title-color flex flex-wrap p-2 gap-1 lg:gap-2 justify-center ">
      {HomepageCardsData.map((data, index) => (
        <div
          key={index}
          className="w-full lg:w-96 border-red-900/80 border-1 rounded-md shadow-md  "
        >
             ...
             <div className="w-20 flex items-center justify-center">
              ...
              <div>
                {data.items.map((dataItem, index) => (
                  <div key={index}>
                    <a href="#" className="hover:border-b-1 transition-all ">{dataItem}</a>
                  </div>
                ))}
              </div>
            </div>
            <div className="text-right px-3 py-1 border-t-1 border-red-900/30">
                <a href="#" className="border-b-1">MORE</a>
            </div>
          </div>
        </div>
      ))}
